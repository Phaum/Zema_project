...
const QuestionaryPage = () => {
  ...
  const [isFetchingCadastral, setIsFetchingCadastral] = useState(false); // Добавьте этот стейт
  ...
  const handleFetchCadastralData = async () => {
    console.log("Кнопка нажата"); // Проверка 1
    try {
      const values = await form.validateFields(['cadastralNumber']);
      console.log("Валидация пройдена, отправляем:", values.cadastralNumber); // Проверка 2
      
      setIsFetchingCadastral(true);

      const response = await axios.post('http://localhost:8080/api/cadastral/info', { 
      ...
      if (response.data.success) {
        const nspd = response.data.data;
      
      // 3. Мапим данные из бэкенда в поля анкеты
        form.setFieldsValue({
        // Пример заполнения адреса, если он пришел
          address: nspd.address,
        // Если твой парсер вернул год постройки, добавь его сюда:
        // yearBuilt: nspd.year_built 
        });

      // Сохраняем данные о местоположении отдельно в state
        setLocationData(prev => ({
          ...prev,
          confirmedAddress: nspd.address,
          metroInfo: nspd.metro // сохраним для отображения в UI
        }));

        message.success('Данные объекта и гео-позиция обновлены');
      }
    } catch (error) {
       console.error("ПОЛНАЯ ОШИБКА:", error); // Это самое важное
                ...
                <Col span={12}>
                  ...
                  >
                    <Input 
                      ...
                      suffix={
                        <Tooltip title="Нажмите для автоматического сбора данных">
                          <CalculatorOutlined 
                            onClick={handleFetchCadastralData} 
                            style={{ color: isFetchingCadastral ? '#1890ff' : 'rgba(0,0,0,.45)', cursor: 'pointer' }} 
                          />
                        </Tooltip>
                      }
                    />
                  ...
                  {/* Инфо-блок под полем ввода, если данные о метро подтянулись */}
                  {locationData.metroInfo && (
                    <div style={{ marginTop: -15, marginBottom: 15, fontSize: '12px' }}>
                      <Text type="secondary">
                        <EnvironmentOutlined /> Ближайшее метро: 
                        <Text strong> {locationData.metroInfo.name}</Text> ({locationData.metroInfo.distance} м)
                      </Text>
                    </div>
                  )}
                </Col>
